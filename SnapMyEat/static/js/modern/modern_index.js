    ...
    if (quantity>0) {
      ...
      cartContainer.innerHTML = `
                <div class="quantity-control">
                  ...
                  <span>${quantity}</span>
                  <button onclick="increaseQuantity(Number(${itemId}),'${value}',true); event.stopPropagation(); suggestion_show_hidden(${itemId});" class="plus">+</button>
                </div> 
            `;  
    }else{
      cartContainer.innerHTML = `
              <button onclick="increaseQuantity(Number(${itemId}), '${value}',true); suggestion_show_hidden(${itemId});" class="add-to-cart">Add to Cart</button>
          `;
    }
  }
  else{
    ...
    if (totalQuantity > 0) {
      ...
      cartContainer.innerHTML = `
              <div class="quantity-control">
                ...
                <button onclick="showPopup(${itemId}); event.stopPropagation(); suggestion_show_hidden(${itemId});" class="plus">+</button>
              </div> 
          `;
    } else {
      cartContainer.innerHTML = `
              <button onclick="showPopup(${itemId}); suggestion_show_hidden(${itemId});" class="add-to-cart">Add to Cart</button>
          `;
    }
  }
}
...
// function updateCartRecomUI(recomItemId) {
//   const itemId = Number(recomItemId);
//   const cartContainer = document.querySelectorAll(`.cart-container-${itemId}-recom`);
  
  
//   if (!cartContainer) return;
  
//   cartContainer.forEach((element)=>{
//     const hiddenDiv  = element.querySelector(".dsdsHidden");
//     const hiddenText = hiddenDiv ? Number(hiddenDiv.textContent.trim()) : null;
    
//     // if (hiddenText!==itemId) return;
//     if (!hiddenText) return;
//     const item = cartItems.find((i) => i.id === hiddenText);
...
//     if (item.oneSize) {
//       const quantity = item.sizes[0].quantity;
//       const value = item.sizes[0].size;

//       if (quantity > 0) {
//         element.innerHTML = `
//         <div class="dsdsHidden" style="display: none;">${itemId}</div>
//           <div class="quantity-control">
//             <button onclick="decreaseQuantity(${itemId}, '${value}', true); event.stopPropagation();" class="minus">-</button>
//             <span>${quantity}</span>
//             <button onclick="increaseQuantity(${itemId}, '${value}', true); event.stopPropagation(); suggestion_show_hidden(${itemId});" class="plus">+</button>
//           </div>`;
//       } else {
//         element.innerHTML = `
//         <div class="dsdsHidden" style="display: none;">${itemId}</div>
//           <button onclick="increaseQuantity(${itemId}, '${value}', true); suggestion_show_hidden(${itemId});" class="add-to-cart">Add to Cart</button>
//         `;
//       }
//     } else {
